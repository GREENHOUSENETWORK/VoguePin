...
Plugin URI: http://wordpress.com/pin-dispenser-for-voguepay
...
if (!class_exists("VoguePayPins")) {
  ...
  class VoguePayPins {
    ...
    <input type="hidden" name="memo" value="Payment for '.$param['item'].'" />';
     ...
     $form .= '<input type="hidden" name="xid_code" value="'.$param['code'].'" />';
...
if ( !function_exists("VoguePayPins_ap") ) { 
    function VoguePayPins_ap() { 
        ...
        if ( function_exists('add_options_page') )  add_options_page('Vogue PIN Dispenser', 'Vogue PIN Dispenser', 9, basename(__FILE__), array(&$v_VoguePayPins, 'echoSettingPage'));
    }   
}
